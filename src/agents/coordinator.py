...
class MultiAgentCoordinator:
    ...
    def process_file(
        ...
        context = self.ingestion_agent.run(
            context
        )

        context = self.data_quality_agent.run(
            context
        )

        context = self.analytics_agent.run(
            context
        )
    ...
    def finalize(self, context):
        ...
        context = self.analytics_agent.finalize(
            context
        )

        context = self.report_agent.run(
            context
        )
